...
RSS::RSS(int _dimension, Function& _function, Rectangle& _rectangle, StopCriterion& _stopCriterion, Vector _initial, double _p) :
...
void RSS::optimize()
{
	...
	double value_next = 0;
	Vector left = rectangle.getLeft();
	Vector right = rectangle.getRight();
	while (!(stopCriterion.ifStop(x_current, x_previous)))
	{
		for (int j = 0; j < dimension; ++j)
			x_next[j] = (left[j] - right[j]) * mtunif() + left[j];
